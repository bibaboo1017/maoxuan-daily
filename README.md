...
句库为 quotes.json，40 条轮换，专用于交易心态：独立判断、克服过度自信、
接受市场变化、面对亏损、停止报复性交易、知错能改、耐心等待、尊重验证、
保持谦逊、执行一致。所有毛选原文都附篇名、日期、上下文和链接。
交易应用由编写者另写，原著本身并不讨论股票，不把交易建议冒充毛泽东原话。
不包含个股推荐、买卖价位或收益承诺。每天换一条，40 天重复一次，并非自动联网扩充。
本次新增 30 条短摘录已与所链接的原文逐条核对；部分摘录为句中片段，需结合原文背景阅读。
...
需要额外推送时，在 Actions → 毛选每日微信推送 → Run workflow 中勾选
“额外推送一次（今天已发过也发送）”。每次新建的补推任务额外发送当天的一条内容；
重跑同一任务不会重复提交，也不影响正常每日记录。定时运行不会启用额外推送。
命令行等价方式为 `python daily.py --send --extra-id 唯一编号`，编号使用英文字母、数字、下划线或连字符。
...
2026-09-19：句库扩充至 40 条交易心态内容；新增有独立防重复记录的额外推送入口。
